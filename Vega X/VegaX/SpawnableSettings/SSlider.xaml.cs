...
{
	public partial class SSlider : UserControl
	{
		...
		public SSlider(string RegistryName, string RealLogo, string RealTitle, string RealSubTitle, string Teller)
		{
			SSlider <>4__this = this;
			this.InitializeComponent();
			this.Logo.Content = RealLogo;
			this.Title.Content = RealTitle;
			this.SubTitle.Content = RealSubTitle;
			this.Slider.Value = Convert.ToDouble(HandleSettings.ReadSString(RegistryName));
			this.SliderTeller.Content = this.Slider.Value.ToString() + " " + Teller;
			this.Slider.ValueChanged += delegate(object s, RoutedPropertyChangedEventArgs<double> e)
			{
				if (<>4__this.SliderTeller != null)
				{
					<>4__this.SliderTeller.Content = <>4__this.Slider.Value.ToString() + " " + Teller;
				}
			};
		}
